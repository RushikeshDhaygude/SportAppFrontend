        ...
        children: [
          Row(
            children: [
              // Team 1 Logo
              ClipOval(
                child: result['team1']['teamLogoPath'] != null
                    ? Image.network(
                        result['team1']['teamLogoPath'],
                        width: 50,
                        height: 50,
                        fit: BoxFit.cover,
                      )
              ...
              // Team 2 Logo
              ClipOval(
                child: result['team2']['teamLogoPath'] != null
                    ? Image.network(
                        result['team2']['teamLogoPath'],
                        width: 50,
                        height: 50,
                        fit: BoxFit.cover,
                      )
  ...
  void openResultForm(BuildContext context, {Map<String, dynamic>? result}) {
    showDialog(
      ...
      builder: (context) => ResultFormDialog(
        ...
        result: result,
        onSave: (result != null)
            ? (resultData) => editResult(result['id'].toString(), resultData)
            : addResult,
      ),
    );
  }
...
class _ResultFormDialogState extends State<ResultFormDialog> {
  ...
  late TextEditingController _eventLocationController;
  late TextEditingController _eventDateTimeController;
  late TextEditingController _team1IdController; // Controller for Team1 ID
  late TextEditingController _team2IdController; // Controller for Team2 ID
  late String _status = 'FINAL'; // Default status
  late DateTime _selectedDate;
  late TimeOfDay _selectedTime;
  List<String> _statusOptions = [
  ...
  void initState() {
    ...
    _team2IdController = TextEditingController(
        text: widget.result?['team2']['id']?.toString() ?? '');
    _status = widget.result?['status'] ?? 'FINAL'; // Set default or existing status
